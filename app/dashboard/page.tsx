...
type Entry = Database["public"]["Tables"]["entries"]["Row"] & {
  competitions: { title: string; status: string; end_date: string }
}
type Dividend = {
  id: string
  amount: number
...
export default function DashboardPage() {
  ...
  const checkUser = async () => {
    try {
      ...
      // Fetch user entries with competition details
                      ...
                      >
                        <div className="flex-1">
                          <div className="flex items-center gap-3 mb-2">
                            <h3 className="font-semibold text-luxury-white">{entry.competitions?.title}</h3>
                  ...
                  {dividends.map((dividend) => (
                    ...
                    >
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-medium text-luxury-white">{dividend.period}</p>
                          <p className="text-sm text-gray-400">
                            {dividend.status === "paid"
                              ? `Paid on ${new Date(dividend.paid_at).toLocaleDateString()}`
                              : "Payment pending"}
                          </p>
                        </div>
                      </div>
                      ...
                      {referrals.map((referral) => (
                        ...
                        >
                          <div className="flex items-center gap-3">
                            ...
                            <div>
                              <p className="font-medium text-luxury-white">{referral.referee_email}</p>
                              <p className="text-sm text-gray-400">
                                Joined {new Date(referral.joined_at).toLocaleDateString()} • Spent £
                          ...
                          <div className="flex items-center gap-3">
                            <div className="text-right">
                              <p className="font-semibold text-gold-500">£{referral.earnings.toFixed(2)}</p>
